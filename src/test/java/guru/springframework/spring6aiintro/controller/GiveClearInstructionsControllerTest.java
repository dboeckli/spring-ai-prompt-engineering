...
class GiveClearInstructionsControllerTest {
    ...
    void createReview_Success() {
        ...
        when(giveClearInstructionsService.cookASteak()).thenReturn(chatResponse);

        ResponseEntity<String> response = giveClearInstructionsController.createReview();

        assertEquals(200, response.getStatusCode().value());
        assertEquals(expectedResponse, response.getBody());
        verify(giveClearInstructionsService).cookASteak();
    }
